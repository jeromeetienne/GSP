...
class DiffableNdarraySerialisation:
    ...
    # =============================================================================
    @staticmethod
    def to_json(diff_ndarray: DiffableNdarray, diff_db: DiffableNdarrayDb) -> dict[str, Any]:
        """
        ...
        assert diff_ndarray.is_modified() is True, "There should be modifications to serialize"
        ...
        assert diff_data is not None, "diff region should not be None if delta slices exists"
    ...
    @staticmethod
    def from_json(json_dict: dict[str, Any], diffable_db: DiffableNdarrayDb) -> "DiffableNdarray":
        """
        ...
        is_patch_full_data = json_dict["slices"] is None and json_dict["data"] is not None
        is_patch_diff_data = json_dict["slices"] is not None and json_dict["data"] is not None
        is_patch_empty = json_dict["slices"] is None and json_dict["data"] is None

        # Sanity check - only one of the above should be true
        assert sum([is_patch_full_data, is_patch_empty, is_patch_diff_data]) == 1, "PANIC is_patch_* logic error"
        ...
        if is_patch_empty:
            if not is_in_db:
                raise DiffableNdarraySerialisationError("DiffableNdarray not found in database, cannot apply empty patch")
            else:
        ...
        if is_patch_diff_data:
            if not is_in_db:
                raise DiffableNdarraySerialisationError("DiffableNdarray not found in database, cannot apply diff patch")
            else:
                # get the existing array as from the database
                existing_arr = diffable_db.from_json_db[json_dict["uuid"]]
                # deserialize the slices and the delta region
                diff_slices = DiffableNdarraySerialisation._slices_from_json(json_dict["slices"])
        ...
        raise DiffableNdarraySerialisationError("DiffableNdarray - Invalid patch format")

    
    @staticmethod
    def get_patch_type(json_dict: dict[str, Any]) -> Literal["full_data", "diff_data", "empty"]:
        """
...
if __name__ == "__main__":
    ...
    deserialized_arr1 = DiffableNdarraySerialisation.from_json(serialized_arr, diffable_ndarray_db)
    ...
    # Deserialize back to DiffableNdarray
    deserialized_arr2 = DiffableNdarraySerialisation.from_json(serialized_arr2, diffable_ndarray_db)
