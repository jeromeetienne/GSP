...
def test_delta_ndarray_setitem_and_delta_tracking():
    arr = DeltaNdarray(np.zeros((4, 4), dtype=int))
    assert arr.get_delta_slices() is None
    arr[1, 2] = 7
    arr[2, 1] = 8
...
def test_delta_ndarray_clear_delta():
...
def test_delta_ndarray_patch_and_json_serialization():
...
def test_delta_ndarray_full_serialization_when_no_modifications():
...
def test_delta_ndarray_slice_to_json_and_from_json():
